...
USO (cada 1-2 semanas):
  1. Entra a MELI Axis > Mi portafolio > exporta "lista_projetos.csv" (vista completa).
  2. Sobrescribe el archivo insights_axis/lista_projetos.csv con el nuevo export.
  3. Corre:  python3 build_insights_mx.py
  4. Se regenera insights_axis/Insights_Portafolio_MX.html con los datos frescos.
  5. Abre ese HTML localmente (doble clic) para la reunion / lo compartes como
     archivo adjunto. NO se sube al Hub publico ni se hace commit al repo:
     este archivo vive solo en tu carpeta local, porque trae datos de
     proyectos de otros equipos (no solo el tuyo).

No requiere Sheets, GCP, ni cuenta de servicio: es 100% local.
"""
...
HTML_OUT = HERE / "Insights_Portafolio_MX.html"
...
<head>
<meta charset="UTF-8">
<title>Insights de Portafolio — OE MLM (México, datos reales de MELI Axis)</title>
<style>
  :root{
  ...
  *{box-sizing:border-box;}
  ...
  .priv-badge{display:inline-block;margin-top:8px;margin-left:8px;background:#3a0f0f;color:#ff8b8b;border:1px solid #ff8b8b;
    padding:5px 12px;border-radius:20px;font-size:12px;}
  main{padding:26px 32px;max-width:1300px;margin:0 auto;}
  .findings{background:var(--panel);border-left:4px solid var(--yellow);border-radius:6px;padding:16px 20px;margin-bottom:24px;}
  ...
  .kpi{background:var(--panel);border-radius:8px;padding:16px;text-align:center;border:1px solid var(--border);}
  .kpi .val{font-size:26px;font-weight:700;color:var(--yellow);}
  ...
  .filters{display:flex;flex-wrap:wrap;gap:12px;align-items:flex-end;background:var(--panel);
    padding:14px 16px;border-radius:8px;margin-bottom:14px;border:1px solid var(--border);}
  .filters label{font-size:11px;color:var(--muted);display:block;margin-bottom:4px;}
  select, input[type=text]{background:var(--panel2);color:var(--text);border:1px solid var(--border);border-radius:5px;padding:6px 8px;font-size:13px;}
  .chkwrap{display:flex;align-items:center;gap:6px;font-size:13px;}
  ...
  .tablewrap{max-height:520px;overflow-y:auto;border-radius:8px;border:1px solid var(--border);}
  table{width:100%;border-collapse:collapse;background:var(--panel);font-size:12.5px;}
  th{background:var(--panel2);text-align:left;padding:9px 10px;color:var(--yellow);font-size:11px;text-transform:uppercase;
...
<header>
  ...
  <div class="sub">Generado con el export "vista completa" de MELI Axis (lista_projetos.csv) — acceso de Visualizador a nivel organización, filtrado a proyectos con CAD de México.</div>
  <div class="scope-badge">✓ Alcance real: __TOTAL__ proyectos de México</div>
  <div class="priv-badge">🔒 Documento privado — no publicar en el Hub</div>
</header>
<main>
  <div class="findings">
    <h2>Hallazgos clave (borrador para el senior manager)</h2>
...
<footer>Demo interno OE MLM · datos exportados de MELI Axis (vista completa, rol Visualizador) · filtrado a CAD de México · generado localmente, no publicado</footer>
<script>
...
function renderKpis(){
  ...
  document.getElementById('findingsList').innerHTML = `
    ...
    <li>El acceso de Visualizador en Axis cubre <strong>${centros} centros/CAD distintos</strong> de México.</li>
